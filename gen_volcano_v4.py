...
CRATER_OFFS = 10                # crater is this many px below the polygon base-y reference
...
# ── Volcano polygon ─────────────────────────────────────────────────────────
# Top of polygon IS the crater rim (dy = CRATER_OFFS from VOL_TIP_Y reference).
# No vertex exists above the crater level — tip is chopped off.
# Reference point: VOL_TIP_Y is set so the crater sits at a natural height.
VOL_TIP_Y = ISLAND_CY - 5 - 40   # same as before; crater at VOL_TIP_Y + CRATER_OFFS

_VOL_POLY = [
    # Top edge = crater opening (left rim → right rim)
    ( -8,  10),   # left crater rim  ← TOP of volcano, no point above this
    (-14,  16),
    (-19,  22),
    (-24,  28),
    (-30,  34),
    ...
    ( 18,  22),
    ( 13,  17),
    (  8,  10),   # right crater rim ← other side of top
]
...
def draw_island(d, sx=0, sy=0):
    """Sand and grass base only — no trees here."""
...
def draw_volcano(d, sx=0, sy=0):
    """Volcano cone: polygon top IS the crater — no tip above it."""
    cx  = ISLAND_CX + sx
    ty  = VOL_TIP_Y + sy
    # Main silhouette
    ...
    # Crater opening — drawn into the top of the cone, not above it
    cr_y = ty + CRATER_OFFS
...
# Back trees: drawn BEFORE volcano — appear behind it
_BACK_TREES = [
    (-44, -9, 0.54),
    (-26, -11, 0.50),
    ( 20, -11, 0.52),
...
# Front trees: drawn AFTER volcano — appear in front of its base
_FRONT_TREES = [
    (-50, -2, 0.50),
    (-32, -4, 0.46),
    ( 28, -4, 0.48),
...
def make_frame(sx=0, sy=0, smoke=0, lava_h=0, glow=0.0, frm=0, rain=False):
    ...
    draw_island(d, sx, sy)
    draw_back_trees(d, sx, sy)       # ← BEHIND volcano
    draw_volcano(d, sx, sy)
    draw_front_trees(d, sx, sy)      # ← IN FRONT of volcano
    if lava_h > 0:
...
quantized[0].save(
    "C:/ClaudeCode/volcano_eruption_v4.gif",
...
peak_rgb.save("C:/ClaudeCode/volcano_preview_v4.png")
